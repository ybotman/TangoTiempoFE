...
function ManageLocations() {
    ...
    const handleSave = async () => {
        try {
            ...
            console.error(`Error ${isCreating ? 'creating' : 'updating'} location:`, error);
        }
    };
    ...
    return (
        <Box sx={{ padding: 2 }}>
            ...
            {locations.map((location) => (
                <Box key={location._id} sx={{ display: 'flex', alignItems: 'center', marginBottom: 2 }}>
                    <Typography variant="subtitle1" sx={{ flex: 1 }}>{location.name}</Typography>
                    <Typography variant="subtitle1" sx={{ flex: 1 }}>{location.city}</Typography>
                    <Typography variant="subtitle1" sx={{ flex: 1 }}>{location.state}</Typography>
                    ...
                    <Switch
                        ...
                        color="primary"
                    />
                    <Button size="small" onClick={() => handleOpen(location)}>View / Edit</Button>
                </Box>
            ))}
