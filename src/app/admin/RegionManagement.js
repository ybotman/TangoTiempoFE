...
function RegionManagement() {
    ...
    useEffect(() => {
        async function fetchRegions() {
            try {
                const response = await axios.get(`${process.env.NEXT_PUBLIC_BE_URL}/api/regions`);
                            ...
                            <Box key={division._id} sx={{ paddingLeft: 4, marginBottom: 2 }}>
                                <Typography variant="subtitle1">
                                    ...
                                    <Switch
                                        ...
                                        onChange={() => handleToggleDivisionActive(region._id, division._id)}
                                    />
                                </Typography>
                                {division.majorCities.map((city) => (
                                    <Box key={city._id} sx={{ paddingLeft: 8 }}>
                                        <Typography variant="body1">
                                            ...
                                            <Switch
                                                checked={city.active}
                                                onChange={() => handleToggleCityActive(region._id, division._id, city._id)}
                                            />
                                        </Typography>
                                    </Box>
                                ))}
