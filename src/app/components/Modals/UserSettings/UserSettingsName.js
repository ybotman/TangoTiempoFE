...
const UserSettingsName = ({ userData, updateUserData }) => {
  ...
  const [division, setDivision] = useState(
    userData?.localUserInfo?.userDefaults?.division?._id || ''
  );
  const [city, setCity] = useState(
    userData?.localUserInfo?.userDefaults?.city?._id || ''
  );
  ...
  useEffect(() => {
    ...
    const isRegionModified =
      region !== (userData?.localUserInfo?.userDefaults?.region || '');
    const isDivisionModified =
      division !== (userData?.localUserInfo?.userDefaults?.division?._id || '');
    const isCityModified =
      city !== (userData?.localUserInfo?.userDefaults?.city?._id || '');
    setIsModified(
      isNameModified || isRegionModified || isDivisionModified || isCityModified
    );
  ...
  return (
    <Box sx={{ mt: 2 }}>
      <Typography variant="h6">
        Update Your Name and Default Location
      </Typography>
      <TextField
        label="First Name"
        value={first}
