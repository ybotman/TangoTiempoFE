...
try {
  ...
  const lastVersion = versions.length > 0 ? versions[versions.length - 1].version : '1.0.0';

  // Remove any existing "local" entry
  const filteredVersions = versions.filter((v) => v.version !== lastVersion || v.branch !== 'local');
